...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedRole) {
      ...
      const savedView = localStorage.getItem('promptmetal_view') as ViewState;
      if (savedView) setCurrentView(savedView);
      const savedProjectId = localStorage.getItem('promptmetal_selected_project');
  ...
  const handleSetRole = useCallback((role: UserRole, stayLoggedIn: boolean) => {
    ...
    if (stayLoggedIn && role) {
      ...
      localStorage.setItem('promptmetal_view', currentView);
      if (selectedProjectId) {
        localStorage.setItem('promptmetal_selected_project', selectedProjectId);
      }
    }
  ...
  const handleDeleteTimesheetBatch = useCallback((ids: string[]) => {
    ids.forEach(id => databaseService.deleteTimesheet(id));
    setTimesheetRecords(prev => prev.filter(r => !ids.includes(r.id)));
  }, []);
  ...
  const handleSelectProject = useCallback((projectId: string) => {
    ...
    if (userRole) {
      localStorage.setItem('promptmetal_selected_project', projectId);
      localStorage.setItem('promptmetal_view', 'project-detail');
    }
  }, [userRole]);
  ...
  const handleLogout = useCallback(() => {
    ...
    localStorage.removeItem('promptmetal_view');
    localStorage.removeItem('promptmetal_selected_project');
  }, []);

  const handleViewChange = useCallback((view: ViewState) => {
    setCurrentView(view);
    if (userRole) {
      localStorage.setItem('promptmetal_view', view);
    }
  }, [userRole]);
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'team': return <TeamView userRole={userRole} onAddEmployee={handleAddEmployee} onUpdateEmployee={handleUpdateEmployee} onDeleteEmployee={handleDeleteEmployee} employees={employees} projects={projects} timesheetRecords={timesheetRecords} />;
      case 'timesheet': return <TimesheetView employees={employees} projects={projects} records={timesheetRecords} onAddBatch={handleAddTimesheetBatch} onDeleteRecord={handleDeleteTimesheet} onDeleteBatch={handleDeleteTimesheetBatch} userRole={userRole} />;
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <Sidebar currentView={currentView} onChangeView={handleViewChange} userRole={userRole} onLogout={handleLogout} isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} />
