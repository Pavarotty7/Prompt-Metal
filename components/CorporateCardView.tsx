...
const CorporateCardView: React.FC<CorporateCardViewProps> = ({ employees, onUpdateEmployee, userRole }) => {
  ...
  const handleSaveTransaction = async (e: React.FormEvent) => {
    ...
    const receiptUrl = receiptFile ? await fileEncodingService.fileToDataUrl(receiptFile) : undefined;
  ...
  const handleAddNewHolder = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin || !onUpdateEmployee) return;
    const target = employees.find(emp => emp.id === cardFormData.employeeId);
    if (target) {
      const cardImage = cardImageFile ? await fileEncodingService.fileToDataUrl(cardImageFile) : undefined;
  ...
  const handleUpdateHolder = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdmin || !onUpdateEmployee) return;
    const target = employees.find(emp => emp.id === cardFormData.employeeId);
    if (target) {
      const cardImage = cardImageFile ? await fileEncodingService.fileToDataUrl(cardImageFile) : target.cardImage;
                      ...
                      {employeeTransactions.length > 0 ? employeeTransactions.map(tx => (
                        <tr key={tx.id} className="hover:bg-slate-50 transition-all group">
                          ...
                          <td className="py-4 px-6 text-center">
                            <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border ${tx.receiptAttached ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                              }`}>
