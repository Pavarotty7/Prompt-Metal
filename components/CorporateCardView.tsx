...
const CorporateCardView: React.FC<CorporateCardViewProps> = ({ employees, onUpdateEmployee, userRole }) => {
    ...
    try {
      ...
      if (editingTransactionId) {
        setTransactions(prev => prev.map(tx => {
          if (tx.id === editingTransactionId) {
            return {
              ...
              receiptAttached: receiptFiles.length > 0 || tx.receiptAttached,
              attachments: [...(tx.attachments || []), ...newAttachments]
            };
          }
          return tx;
        }));
        ...
        const newTx: CorporateCardTransaction = {
          ...
          category: txFormData.category,
          status: 'Pendente',
          receiptAttached: newAttachments.length > 0,
  ...
  const handleEditTransaction = (tx: CorporateCardTransaction) => {
    ...
    <div className="space-y-6 animate-fade-in pb-10">
                     ...
                     <thead className="bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th className="py-4 px-6 text-[9px] font-black uppercase text-black tracking-widest text-center">Documento</th>
                             {isAdmin && <th className="py-4 px-6 text-[9px] font-black uppercase text-black tracking-widest text-center">Ações</th>}
                        </tr>
                     </thead>
                         ...
                         <tr key={tx.id} className="hover:bg-slate-50 transition-all group">
                            ...
                            <td className="py-4 px-6 text-center">
                                 <div className="flex items-center justify-center gap-1">
                                     ...
                                     {(!tx.attachments || tx.attachments.length === 0) && !tx.receiptUrl && (
                            ...
                            {isAdmin && (
                              <td className="py-4 px-6 text-center">
                                 <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                     <button onClick={() => handleEditTransaction(tx)} className="p-2 text-slate-400 hover:text-blue-600 transition-all"><Edit size={14} /></button>
                                     <button onClick={() => handleDeleteTransaction(tx.id)} className="p-2 text-slate-400 hover:text-red-600 transition-all"><Trash2 size={14} /></button>
                                 </div>
                              </td>
                            )}
                         </tr>
                           ...
                           <tr>
                               <td colSpan={7} className="py-24 text-center text-slate-300 text-[10px] font-black uppercase italic tracking-[0.2em]">Nenhum lançamento para este período.</td>
                           </tr>
                       )}
                     </tbody>
                   </table>
      ...
      {isTransactionModalOpen && isAdmin && selectedEmployee && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[150] flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/20 animate-scale-in">
              <div className="bg-slate-900 p-8 flex justify-between items-center">
                 <h3 className="text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-3"><Coins size={24} className="text-amber-500" /> {editingTransactionId ? 'Editar Despesa' : 'Nova Despesa'}</h3>
                 <button onClick={() => { setIsTransactionModalOpen(false); setEditingTransactionId(null); }} className="bg-white/10 p-2 rounded-full text-white hover:bg-white/20 transition-all"><X size={24} /></button>
              </div>
              <form onSubmit={handleSaveTransaction} className="p-8 space-y-6">
                 ...
                 <div>
                    <label className="block text-[10px] font-black text-black uppercase mb-2 tracking-widest">Upload de Faturas/Recibos (Múltiplos)</label>
                    <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 bg-slate-50 relative group flex flex-col items-center justify-center">
                        <input type="file" multiple className="absolute inset-0 opacity-0 cursor-pointer" onChange={e => setReceiptFiles(Array.from(e.target.files || []))} />
                       ...
                       <p className="text-[9px] font-black text-black uppercase tracking-widest">
                         {receiptFiles.length > 0 
                           ? `${receiptFiles.length} arquivos selecionados` 
                           : 'Selecionar Documentos'}
                       </p>
                    </div>
                 </div>
                 <div className="pt-6 flex justify-end gap-3 border-t border-slate-100">
                    <button type="button" onClick={() => { setIsTransactionModalOpen(false); setEditingTransactionId(null); }} className="px-6 py-3 text-slate-400 font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 rounded-2xl transition-all">Cancelar</button>
                    <button type="submit" disabled={isUploading} className="bg-slate-900 text-white px-10 py-4 rounded-2xl text-sm font-black uppercase tracking-widest shadow-xl hover:bg-black active:scale-95 transition-all flex items-center gap-2">
                       {isUploading ? (
                         ...
                         editingTransactionId ? 'Atualizar Lançamento' : 'Confirmar Lançamento'
                       )}
                    </button>
                 </div>
              </form>
