...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ project, onBack, userRole, employees = [], onUpdateProject }) => {
  ...
  const handleSaveMaterial = async (e: React.FormEvent) => {
    ...
    const attachmentUrl = materialAttachment ? await fileEncodingService.fileToDataUrl(materialAttachment) : undefined;
  ...
  const handleSaveDoc = async (e: React.FormEvent) => {
    ...
    if (docAttachment) {
      try {
        localUrl = await fileEncodingService.fileToDataUrl(docAttachment);
